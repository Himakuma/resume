...
|VBScript|1年程度|-||
|JavaScript|8年以上|jQuery、react|HTML、CSSも同じレベルの経験|
|Objective-C|半年未満|||
|Swift|半年未満|||
<br/>
...
* UnityのUI制御処理の実装方法の検討・設計・実装
    * 画面単位に対応するクラス定義をする
    * UIの管理クラスにて、画面とクリック等のイベントを自動的に紐づけする
* アプリへOVRLipsyncの組み込み
* 新端末、ライブラリ等のフィジビリティ調査・検証
* マルチプラットフォーム向けの各ネイティブプラグイン作成
    * メイン処理に影響がないように、プラットフォーム単位の実装部分の構成設計・実装
    * iOSのSwiftライブラリ連携用のUnity向けネイティブプラグイン作成
